...
use std::path::Path;
#[cfg(feature = "nextcloud")]
use cloud_sync_lib::{NextcloudProvider, StorageBackend};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let temp_file_path = "nextcloud_test_connection_tmp.txt";
    println!("\n2. Performing read/write test (Upload -> List -> Delete)...");
    
    // Create a local temporary file
    {
        let mut file = File::create(temp_file_path)?;
        writeln!(file, "Nextcloud connectivity check file. Created at: {:?}", std::time::SystemTime::now())?;
    }
    ...
    println!("\n🎉 All Nextcloud connectivity tests passed successfully!");
    Ok(())
}
