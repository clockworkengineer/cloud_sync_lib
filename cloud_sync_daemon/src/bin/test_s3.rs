...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match provider.list("").await {
        Ok(items) => {
            println!("   -> Success! Found {} items in bucket.", items.len());
    ...
    let temp_file_path = "s3_test_connection_tmp.txt";
    println!("\n2. Performing read/write test (Upload -> List -> Delete)...");
    
    // Create a local temporary file
    {
        let mut file = File::create(temp_file_path)?;
        writeln!(file, "S3 connectivity check file. Created at: {:?}", std::time::SystemTime::now())?;
    }
